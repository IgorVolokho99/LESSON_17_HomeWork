...
class MoviesSchema(Schema):
    ...
    director_name = fields.Nested("DirectorSchema", only=("name",))
    genre_name = fields.Nested("GenreSchema", only=("name",))


api = Api(app)
movie_ns = api.namespace("movies")
directo_ns = api.namespace("directors")
gentre_ns = api.namespace("genres")
...
class DirectorView(Resource):
    ...
    def put(self, uid:int):
        ...
        if "name" in request_json:
            director.name = request_json.get()
...
class GenresView(Resource):
    def get(self):
        all_genres = db.session.query(Director)
